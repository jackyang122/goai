...
Production **Python 3.12+ / FastAPI** backend that implements the full protocol in
...
> **版本管理用 [`uv`](https://docs.astral.sh/uv/)**:`uv.lock` 锁定整棵依赖树(226 包,含哈希),保证可复现。
> `uv` 会自动准备 Python 3.12 解释器,无需手动安装。`requires-python>=3.12`(litellm/fsrs 等当前版本均需 ≥3.10)。
...
```bash
# 1. 装依赖到 uv 管理的 .venv（自动准备 Python 3.12，按 uv.lock 锁定版本）
#    默认只装核心 + dev；按需加 --extra：llm/embed/rag/mem0/bkt/fsrs/parse 或 --all-extras
uv sync --extra dev
...
uv run python -m plos.cli seed

# 5. 起服务
uv run uvicorn plos.app.main:app --port 8001 --reload
#   健康检查：GET http://localhost:8001/api/health
```

> 不用 uv 也可以:`python -m venv .venv && .venv/Scripts/activate && pip install -e ".[all,dev]"`(Python 3.12+)。
> 升级依赖:`uv lock --upgrade`(或 `--upgrade-package litellm` 单升一个),提交新的 `uv.lock`。
...
```bash
uv run python -m plos.cli run                 # 等同 uv run uvicorn plos.app.main:app
uv run python -m plos.cli seed                # 幂等种子
uv run python -m plos.cli recompute-mastery   # 用闭式重算所有主题 trend
uv run python -m plos.cli recompute-mastery --fit   # 离线 pyBKT 拟合重标定 mastery_params（仅离线！）
```
...
```bash
uv run pytest                # DB-free 逻辑测试（schema 判别联合 / 批改 / FSRS / 路由 / BKT）
```
